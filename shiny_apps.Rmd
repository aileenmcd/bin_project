...
shinyApp(
  ...
  ui = fluidPage(
    ...
    column(3, sliderInput("date_chosen", "Days:", 
                          ...
                          median(unique(combined_bin_osm$cumul_date)), 
...
ggplot() +
  ...
  geom_sf(data = streets_simplified %>% 
          ...
          size = .5,
          color = "grey30") +
  geom_sf(data =   subset_bin_data(), color = "red", size = .5, show.legend = "line") +
  ...
  coord_sf(ylim = c(min_max_coords[1], min_max_coords[3]),
    ...
    output$street_plot = renderPlot({
       ...
       subset_bin_data() %>%
        ...
        geom_col() +
        scale_y_continuous(breaks = c(120, 983) , labels = c('baby elephant', 'vauxhall corsa'))
