...
The scripts to clean and tidy the bin data and OSM data and join these together as in the 'cleaning_scripts' folder.  The cleaned data is then saved in the folder 'cleaned_data'. 
...
There are 2 pieces of work for analysing & visualising the data (which are still works in process):

* Markdown report with some visuals on top and bottom streets in terms of cumulative rubbish weight, spatial visualisation of streets in terms of rubbish weight and looking at the rate of change of rubbish accumulating on particular streets. 
* Shiny app where the user can select 2 streets and compare the total weight, how the weight accumulated over time and where the streets are located spatially. The user can select for the graphs to be in kilograms or equivalent weight object (as an alternative way of conceptualising the weight of the rubbish). The app is also hosted [here]() if don't want to run the code. 
...
![](output/analysis_report/plot_images/top_street_objects.png){ width=10% height=10% }
